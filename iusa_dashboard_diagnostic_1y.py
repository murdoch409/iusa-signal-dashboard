...
# Streamlit app
st.set_page_config(page_title='IUSA Signal Dashboard (Diagnostic)', layout='wide')
st.title('IUSA Buy/Hold/Sell Signal — DIAGNOSTIC MODE')
...
try:
    with st.spinner('Fetching and preparing data...'):
        ...
        df.dropna(subset=['RSI', 'MACD', 'Signal_Line', '50_MA', '200_MA'], inplace=True)
        ...
        if df.empty:
            st.warning("⚠️ Not enough valid data to display charts. Try using a longer period or wider time interval.")
        else:
