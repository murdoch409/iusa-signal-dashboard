...
# Add indicators and ensure they are flat 1D arrays
def add_indicators(df):
    df['RSI'] = pd.Series(np.ravel(ta.momentum.RSIIndicator(close=df['Close']).rsi()), index=df.index)
    macd = ta.trend.MACD(close=df['Close'])
    df['MACD'] = pd.Series(np.ravel(macd.macd()), index=df.index)
    df['Signal_Line'] = pd.Series(np.ravel(macd.macd_signal()), index=df.index)
    df['50_MA'] = pd.Series(np.ravel(df['Close'].rolling(window=50).mean()), index=df.index)
    df['200_MA'] = pd.Series(np.ravel(df['Close'].rolling(window=200).mean()), index=df.index)
    return df
...
# Streamlit app
st.set_page_config(page_title="IUSA Signal Bulletproof FINAL", layout="wide")
st.title("📈 IUSA Buy/Hold/Sell Signal — Bulletproof FINAL")
