...
# Add indicators and ensure they are 1D
def add_indicators(df):
    df['RSI'] = ta.momentum.RSIIndicator(close=df['Close']).rsi().squeeze()
    macd = ta.trend.MACD(close=df['Close'])
    df['MACD'] = macd.macd().squeeze()
    df['Signal_Line'] = macd.macd_signal().squeeze()
    df['50_MA'] = df['Close'].rolling(window=50).mean().squeeze()
    df['200_MA'] = df['Close'].rolling(window=200).mean().squeeze()
    return df
...
# Streamlit app
st.set_page_config(page_title="IUSA Signal Stable", layout="wide")
st.title("📈 IUSA Buy/Hold/Sell Signal — Simplified & Stable")
