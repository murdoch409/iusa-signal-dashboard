    ...
    with st.spinner('Fetching and preparing data...'):
        ...
        if df.empty:
            ...
            if '50_MA' in df.columns and df['50_MA'].notna().any():
                df_plot_ma50 = df.dropna(subset=['50_MA'])
                ax.plot(df_plot_ma50.index, df_plot_ma50['50_MA'], label='50 MA', linestyle='--')
            if '200_MA' in df.columns and df['200_MA'].notna().any():
                df_plot_ma200 = df.dropna(subset=['200_MA'])
                ax.plot(df_plot_ma200.index, df_plot_ma200['200_MA'], label='200 MA', linestyle='--')
            ax.set_title('IUSA Price with Moving Averages')
            ax.legend()
            st.pyplot(fig)

            fig2, ax2 = plt.subplots(figsize=(14, 4))
            df_plot_macd = df.dropna(subset=['MACD', 'Signal_Line'])
            ax2.plot(df_plot_macd.index, df_plot_macd['MACD'], label='MACD')
            ax2.plot(df_plot_macd.index, df_plot_macd['Signal_Line'], label='Signal Line')
            ax2.set_title('MACD Indicator')
            ax2.legend()
            st.pyplot(fig2)

            fig3, ax3 = plt.subplots(figsize=(14, 2))
            df_plot_rsi = df.dropna(subset=['RSI'])
            ax3.plot(df_plot_rsi.index, df_plot_rsi['RSI'], label='RSI', color='purple')
