...
st.set_page_config(page_title="IUSA AutoFix Cleaned", layout="wide")
st.title("📈 IUSA Buy/Hold/Sell Signal — AutoFix Cleaned")
...
def fetch_data():
    ...
    # If MultiIndex columns like ('Close', 'IUSA.L'), flatten them
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(0)
    
    df.dropna(inplace=True)

    # Rename columns if 6 columns returned
    if len(df.columns) == 6:
